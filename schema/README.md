# hacienda-mcp config schemas

Each file in this directory is a **versioned JSON Schema (Draft 2020-12)**
snapshot for a major version of the hacienda-mcp config format. The schemas are
...
in `hacienda-mcp-config-v1.schema.json` is regenerated from
...
- New schema = new file. `hacienda-mcp-config-v1.schema.json` is immutable once
  shipped; v2 lands as `hacienda-mcp-config-v2.schema.json`.
...
- Migration between versions lives in `src/config/migrate.rs`. v1 is the
  ...
  step and wire it into the chain — and expose a `hacienda-mcp config migrate`
