---
title: Architecture
description: How hacienda-mcp is built — from scanning to serving to indexing.
---

import { Aside, FileTree } from "@astrojs/starlight/components";

hacienda-mcp is a single Rust crate with one binary (`hacienda-mcp`) serving three roles: **`hacienda-mcp scan`** indexes a workspace, **`hacienda-mcp serve`** runs the MCP stdio server, and the **daemon** (`hacienda-mcp comms daemon`, auto-spawned; singleton per user) is the sole writer managing the global cache.
...
Your project (code + documents + git history)
  ↓
hacienda-mcp scan (or incremental watch)
...
hacienda-mcp persists project state in a machine-global cache under `~/.local/share/hacienda-mcp/` (or `$HACIENDA_MCP_DATA_HOME`), keyed by workspace root. All repos/worktrees on one machine share a single content-addressed blob store and a daemon-managed Fjall writer.

### Content-Addressed Msgpack Blobs

Location: `~/.local/share/hacienda-mcp/blobs/<hash>.{l1,l2,l3}.msgpack`
...
Location: `~/.local/share/hacienda-mcp/views/<workspace_hash>/<view>/index.fjall/`

A disk-backed, append-only LSM tree (Fjall) provides lightning-fast lookups. The daemon is the sole
writer; all `hacienda-mcp serve` sessions read concurrently from the same index via a read-only handle:
...
Schema version is stamped in `meta`. On version mismatch, the next `hacienda-mcp scan` auto-wipes and rebuilds.

### LanceDB Vector Store

Location: `~/.local/share/hacienda-mcp/lancedb/`
...
`hacienda-mcp scan` and `hacienda-mcp watch` (incremental) share the same per-file pipeline:
...
hacienda-mcp uses the **tree-sitter language pack** (TSLP) to detect and parse 300+ languages. Language detection is dynamic:
...
`hacienda-mcp git` commands are powered by `gix` (a pure-Rust Git implementation) with a **git-history index** for microsecond latency:

### Git-History Index

Location: `~/.local/share/hacienda-mcp/git-cache/<workspace_hash>/`

Two-tier caching:

- **In-process LRU** (1024 entries per category by default; tune via `hacienda-mcp serve --git-cache-mem`)
...
**Minor release** (`0.1.x` → `0.2.0`): `RELEASE_MINOR` increments. On next `hacienda-mcp scan`, `.hacienda-mcp/` is auto-wiped and rebuilt from source. Users are notified in release notes.

**Wipe-on-mismatch** is the explicit migration story. No manual steps — `hacienda-mcp scan` handles it.

## Directory Layout

Global cache at `~/.local/share/hacienda-mcp/` (or `$HACIENDA_MCP_DATA_HOME`):
...
repo/.hacienda-mcp/basemind.toml    # Configuration
```

The daemon manages the global cache; individual repos share no per-repo `.hacienda-mcp/` directory.

## MCP Surface

`hacienda-mcp serve` exposes a stdio MCP server. All paths are byte-precise repo-relative (`RelPath`).
...
<Aside>
  The architecture trades off simplicity for performance: content-addressed
  blobs enable deduplication, the inverted index gives instant lookups, and the
  git-history index makes history queries microsecond-fast. At the same time,
  incremental scanning and a split memory substrate support multi-agent
