...
<Aside type="note">
  All commands support `--json` for machine-readable output. Query and git
  commands take the same filters as their MCP counterparts (`--limit`,
  `--language`, `--path-contains`, …). Cursor-based pagination (`--cursor`) is
  exposed on `comms history` and `comms inbox`.
</Aside>
