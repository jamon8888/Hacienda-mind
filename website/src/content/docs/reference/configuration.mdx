---
title: Configuration
description: Configure hacienda-mcp via config file, environment variables, or per-request overrides.
---

import { Aside, Code } from "@astrojs/starlight/components";

hacienda-mcp merges configuration from five layers. Highest precedence wins: a per-request MCP
...
Create `basemind.toml` at the repo root to customize behavior. `hacienda-mcp init` (see
...
being silently ignored. A legacy `.hacienda-mcp/basemind.toml` path is still read as a fallback for
older checkouts.
...
| `exclude`           | array of globs | build/vendor dirs | Excludes `target/`, `node_modules/`, `dist/`, `.venv/`, `.git/`, `.hacienda-mcp/`, and `bazel-*` trees.                                                                              |
...
| `extra_roots`       | array of paths | `[]`              | Index directories outside the repo root (e.g., a Bazel external cache). External files are keyed by absolute path; (re-)indexed on full `hacienda-mcp scan` only (not live-watched). |
...
[JSON Schema](https://github.com/jamon8888/Hacienda-mind/blob/main/schema/hacienda-mcp-config-v1.schema.json)
...
| `user_agent`            | string  | `hacienda-mcp/<version>` | Identifies the crawler to site operators.                                                   |
| `allow_private_network` | bool    | `false`                  | Allow URLs resolving to private/loopback/link-local addresses. Off by default (SSRF guard). |

### `[pii]` Section

In-process GLiNER2 + Candle (PEFT LoRA) PII redaction. Requires `--features pii`.
...
| Key                | Type | Default   | Notes                                                                             |
| ------------------ | ---- | --------- | --------------------------------------------------------------------------------- |
| `enabled`          | bool | `false`   | Master switch for the PII pass.                                                   |
| `model_dir`        | path | unset     | Directory with `tokenizer.json` + `model.safetensors`. Unset to pass is skipped.  |
...
When `model_dir` is missing or the model fails to load, the pass degrades gracefully (skipped, with a
warning) so a scan never fails on a missing model. See [PII Redaction](/capabilities/pii-redaction).
...
| `agent_id`              | string  | auto    | Stable identity presented to the broker. Falls back to `HACIENDA_MCP_AGENT_ID`, then a generated per-session id. |
...
[llm]
model = "..."
api_key = "sk-..."  # or the HACIENDA_MCP_LLM_API_KEY env var
...
| `api_key`      | string  | API key for the provider. Also read from `HACIENDA_MCP_LLM_API_KEY`.             |
...
Settings map to environment variables by prefixing `HACIENDA_MCP_`, uppercasing, and joining the
section and key with an underscore.

```bash
HACIENDA_MCP_SCAN_EAGER_L2=false
HACIENDA_MCP_SCAN_EXTRA_ROOTS="/path/to/external"
HACIENDA_MCP_DOCUMENTS_ENABLED=true
HACIENDA_MCP_COMMS_AGENT_ID="my-agent"
HACIENDA_MCP_LLM_API_KEY="sk-..."
```

<Aside type="tip">
  Environment variables take precedence over the config file (but CLI flags and
  per-request MCP overrides still win over them). Useful for CI/CD where you
...
When installing hacienda-mcp, select which features to compile.

A vanilla `cargo install hacienda-mcp` builds the code-map and git tools only (default features off, no
...
| `pii`         | In-process GLiNER2 + Candle (PEFT LoRA) PII redaction of PERSON / ORGANIZATION / LOCATION.      | no        |
...
cargo install hacienda-mcp --features full --locked

# Code + git only (smallest build)
cargo install hacienda-mcp --locked

# Code + git + comms
cargo install hacienda-mcp --features comms --locked

# Homebrew / npm / pip (all features by default)
brew install Goldziher/tap/hacienda-mcp
npm install -g hacienda-mcp
pip install hacienda-mcp

# PII redaction (GLiNER2 + Candle + LoRA) alongside the code-map + git tools
cargo install hacienda-mcp --features pii --locked
```
...
hacienda-mcp uses `RELEASE_MINOR` from `src/version.rs` as the single source of truth for both:
...
When either schema changes (typically on minor version bumps), the next `hacienda-mcp scan` automatically wipes the old caches and rebuilds from source. Patch releases are always cache-compatible.

## Directory Layout

All state lives under `.hacienda-mcp/` in the repository root (`--root` selects a different repo):
...
The `.hacienda-mcp/` directory is safe to delete; `hacienda-mcp scan` rebuilds it.
...
**`hacienda-mcp serve` not starting?** Check that `hacienda-mcp` is on your PATH (`which hacienda-mcp`). In `.mcp.json`, use the full path if needed: `"command": "/path/to/hacienda-mcp"`.

**Out of disk space?** Run `hacienda-mcp cache gc` to reclaim unused blobs. `hacienda-mcp cache stats` shows per-component sizes.

**Schema mismatch after upgrade?** Run `hacienda-mcp scan` to automatically rebuild. Or delete `.hacienda-mcp/` and re-scan. No data loss — everything is rebuilt from source.
