...
Measured with the hardening harness, which clones each project fresh, then runs `hacienda-mcp scan`:
...
hacienda-mcp precomputes a per-repo git-history index — posting lists mapping paths to commits (newest-first) — so the history tools are posting-list lookups rather than tree walks.
...
Once the index is in memory (after `hacienda-mcp serve` starts):
...
`hacienda-mcp serve` holds the index in memory so it can answer without re-reading the project. The
footprint scales with project size; `hacienda-mcp cache stats` reports both the on-disk cache
...
<Aside type="note">
All numbers are from production hacienda-mcp builds (`cargo build --release`). Debug builds are much slower and not representative.
</Aside>

## Re-scan Performance

`hacienda-mcp watch` and live-watched re-scans only process changed files:
