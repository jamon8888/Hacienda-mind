...
hero:
  ...
  actions:
    - text: Get started
      ...
      variant: primary
    - text: View on GitHub
      link: https://github.com/jamon8888/Hacienda-mind
      icon: external
      variant: minimal
---
...
## The operating rule: hacienda-mcp first, shell/grep/git fallback

<CardGrid>
  <Card title="Answer from structure" icon="magnifier">
    `outline` a file before you open it, `search_symbols` instead of grep,
    `find_references` instead of grepping call sites. Pointers, not file bodies.
  </Card>
  <Card title="Ask history directly" icon="git-branch">
    `recent_changes`, `blame_symbol`, `commits_touching`, `diff_outline` — git
    intelligence at symbol resolution, in tens of microseconds.
  </Card>
  <Card title="Search everything" icon="open-book">
    Semantic + full-text search over PDFs, Office, HTML, email, and images (OCR)
    — plus scraped web pages — with reranking and per-document summaries.
  </Card>
  <Card title="Coordinate agents" icon="comment">
    A shared channel for a team of agents on the same repo: rooms, inboxes,
    direct messages, and orchestration of many named subagents.
  </Card>
</CardGrid>
...
`hacienda-mcp` program for you on first use. Or install the program directly with a package manager.

<Tabs syncKey="installer">
  <TabItem label="Claude Code">
    <Code
      code={`/plugin marketplace add jamon8888/Hacienda-mind\n/plugin install hacienda-mcp@hacienda-mcp`}
      lang="text"
    />
  </TabItem>
  <TabItem label="Codex">
    <Code
      code={`codex plugin marketplace add jamon8888/Hacienda-mind\ncodex plugin add hacienda-mcp@hacienda-mcp`}
      lang="bash"
    />
  </TabItem>
  <TabItem label="Cursor">
    <Code code={`/add-plugin hacienda-mcp`} lang="text" />
  </TabItem>
  <TabItem label="Gemini CLI">
    <Code
      code={`gemini extensions install https://github.com/jamon8888/Hacienda-mind`}
      lang="bash"
    />
  </TabItem>
  ...
  <TabItem label="Homebrew">
    <Code code={`brew install Goldziher/tap/hacienda-mcp`} lang="bash" />
  </TabItem>
  <TabItem label="npm">
    <Code code={`npm install -g hacienda-mcp`} lang="bash" />
  </TabItem>
  <TabItem label="pip">
...
<CardGrid>
  <LinkCard
    ...
    description="What hacienda-mcp is and why it exists."
  />
  <LinkCard
    title="Quickstart"
  ...
  <LinkCard
    ...
    description="One scan, then instant answers held in memory."
  />
  <LinkCard
    title="MCP tools"
    href="/reference/mcp-tools/"
    description="Every tool, what it returns, and when to reach for it."
  />
  <LinkCard
    title="CLI reference"
    href="/reference/cli/"
    description="The scriptable surface, at 1:1 parity with the MCP tools."
  />
  <LinkCard
    title="Performance"
    href="/reference/performance/"
    description="Scan speed and microsecond git-history queries."
  />
</CardGrid>
