...
title: Introduction
description: >-
  What hacienda-mcp is and why it exists — the context layer that lets agents answer
  from structure instead of burning tokens on grep and whole-file reads.
---

import { Aside, Card, CardGrid, LinkCard } from "@astrojs/starlight/components";

hacienda-mcp turns any repository into an always-current map of its code, documents,
...
## The problem hacienda-mcp solves
...
hacienda-mcp does all three jobs in **milliseconds**, and answers with **paths, line
...
hacienda-mcp is built on one discipline: **prefer structure over content**.
...
- **Do not re-read a file hacienda-mcp already mapped.** If the outline answered the
  question, stop.
...
`hacienda-mcp scan` reads your project once, in parallel, and forwards the result to a
background daemon that manages a machine-global cache under `~/.local/share/hacienda-mcp/`.
All repos/worktrees on one machine share this cache. After scanning, `hacienda-mcp serve`
...
<CardGrid>
  <LinkCard
    ...
    description="Three ways to run hacienda-mcp — pick your tool."
  />
  <LinkCard
    title="Quickstart"
