...
<Aside type="tip">
New to a repo? Run `hacienda-mcp init` (or `/bm-init`) first — see
...
4. **Find who last touched a line**
   ...
   <Code code={`hacienda-mcp git blame-symbol src/main.rs "processFile"`} lang="bash" />
...
<Aside>
After re-indexing, both the CLI (`hacienda-mcp query ...`) and the MCP tools have
