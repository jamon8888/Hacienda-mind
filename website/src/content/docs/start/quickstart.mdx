...
<Aside type="tip">
  New to a repo? Run `hacienda-mcp init` (or `/bm-init`) first — see [Get
  started](/start/installation/#get-started) — to scaffold `basemind.toml` and
  wire agent-instructions rules before you scan.
</Aside>
...
1. **Index the repository**
   ...
   <Code code={`hacienda-mcp scan`} lang="bash" />

   This walks the tree, parses with tree-sitter, and writes a content-addressed
   blob store + inverted index under `.hacienda-mcp/`. Takes a few seconds for small
   repos, around 18 seconds for an 81k-file TypeScript monorepo.

2. **Find where a symbol is defined**

   <Code code={`hacienda-mcp query symbol "parseQuery"`} lang="bash" />

   Output:

   <Code
     code={`src/parser.rs:142:1 parseQuery (function)
...
3. **See what calls a function**
   ...
   <Code code={`hacienda-mcp query references "processFile"`} lang="bash" />

   Output:

   <Code
     code={`src/scanner.rs:142:9 processFile
...
4. **Find who last touched a line**
   ...
   <Code code={`hacienda-mcp git blame-file src/main.rs`} lang="bash" />

   Or drill down to a single symbol:

   <Code
     code={`hacienda-mcp git blame-symbol src/main.rs "processFile"`}
     lang="bash"
   />
...
5. **Keep the index fresh**
   ...
   <Code code={`hacienda-mcp watch`} lang="bash" />

   This runs in the foreground and re-indexes on file changes. Or use `hacienda-mcp
...
- **Outline before you read**: Use `outline` to see a file's structure (symbols,
  ...
  <Code code={`hacienda-mcp query outline src/parser.rs`} lang="bash" />
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="One scan, then instant answers — the architecture behind hacienda-mcp."
  />
  <LinkCard
    title="MCP tools"
...
<Aside>
  After re-indexing, both the CLI (`hacienda-mcp query ...`) and the MCP tools
  have the fresh index. No server restart needed — use `rescan` to update
  in-process.
</Aside>
