...
title: Token economy
description: >
  Why hacienda-mcp saves tokens: pointers (paths and line numbers) cost less than whole files.
  Good habits the plugin sets up by default, plus optional guardrails.
---

hacienda-mcp saves tokens by answering with **file paths, line numbers, and signatures — not whole
files**. A question about your code costs a small fraction of the tokens it takes to read the
source.

## Why pointers beat file reads

When you ask "where is the `User` class defined?", hacienda-mcp returns:
...
When you ask "what calls `processFile`?", hacienda-mcp returns 10 call sites with their paths and
...
See [How it works](/concepts/how-it-works/) to understand how hacienda-mcp builds the map that makes
this possible.
...
- **Don't re-read a file hacienda-mcp already mapped** — use the outline instead.
...
```bash
HACIENDA_MCP_GUARD=off      # disable (default: on)
HACIENDA_MCP_GUARD=redirect # block instead of nudge
```

When an agent reaches for a built-in `Grep`/`Glob`-style search, `Guard` gently redirects it to
the matching hacienda-mcp tool (`workspace_grep` for patterns, `search_symbols` for names,
...
```bash
HACIENDA_MCP_COMPRESS_OUTPUT=1
```
...
```bash
HACIENDA_MCP_DELTA_READS=1
```

When an agent re-reads a file it already read earlier in the session, `HACIENDA_MCP_DELTA_READS` shows
just the changed lines instead of resending the whole file.

## Compression that understands code

hacienda-mcp shrinks code by keeping the shape and dropping the bodies — function signatures and
