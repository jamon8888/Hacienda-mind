...
title: How it works
description: >
  One scan indexes your code and documents; hacienda-mcp serve holds the map in memory
  and answers questions instantly. Re-scans touch only what changed.
---

hacienda-mcp turns your project into an always-current map through two simple steps: a one-time
...
This happens once at setup, then incrementally as you work: when you open a file editor, hacienda-mcp
...
and hacienda-mcp runs hand-tuned queries over that tree to find the pieces you care about.

### Document extraction

If you have PDFs, Office documents, HTML, email, or images, hacienda-mcp extracts their text (with
OCR for images) and indexes their content for semantic search.

## The server

`hacienda-mcp serve` loads the index into memory and exposes it as an MCP server. From here, code
...
When you edit code, hacienda-mcp detects the change and re-scans the affected file. Other files stay
...
| `building_index` | Indexing from scratch (no `.hacienda-mcp/` yet).                             | `true`  |
...
`search_documents`, hacienda-mcp compares your query against the embeddings of every document chunk
