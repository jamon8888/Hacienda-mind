...
hacienda-mcp precomputes a per-repo git-history index backed by `gix`, resolving blame and diffs
...
<Aside type="note">
  Git tools require `hacienda-mcp serve` to be running inside a git repository.
  The history index auto-builds on first use; if it is fresh (matching HEAD),
  the tools use it. Otherwise they walk history directly, so results are always
  current.
</Aside>
...
<Aside type="note">
  All paths are repository-relative. Lists are capped (`limit`, default 100, max
  1000).
</Aside>
