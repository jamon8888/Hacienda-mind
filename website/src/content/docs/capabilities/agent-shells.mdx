...
{
  "text": "echo 'ready'",
  "session_ids": ["abc123def456", "def456ghi789"]
}
```
...
shell_broadcast {
  "text": "cargo build --release -p hacienda-mcp",
...
<Aside type="note">
  Headless sessions have no real terminal — they buffer stdout/stderr and you
  read it with `shell_capture`. For interactive shells (passing `-it` flags) or
  to watch a session's output in real time, the agent can open a tab or window
