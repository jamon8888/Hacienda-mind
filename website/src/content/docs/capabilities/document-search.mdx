...
hacienda-mcp extracts 90+ file formats (PDF, Office, HTML, email, images via OCR) into a
...
- Documents must be scanned first: `hacienda-mcp scan` with the documents feature extracts
  and embeds them into `.hacienda-mcp/`.
...
<Aside type="note">
  The crawler honors `robots.txt` by default (disable with config). It
  SSRF-blocks private and loopback hosts unless `[crawl].allow_private_network =
  true` is set in `.hacienda-mcp/basemind.toml`.
</Aside>
...
<Aside type="note">
  All results are paginated. Use `next_cursor` from one response as `cursor` in
  the next to fetch additional pages.
</Aside>
