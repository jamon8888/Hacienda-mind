...
- SSRF-blocks private and loopback hosts by default to prevent exfil. Allow them with
  `[crawl].allow_private_network = true` in `.hacienda-mcp/basemind.toml`.
...
In `.hacienda-mcp/basemind.toml` (config-file-only):
...
<Aside type="caution">
  SSRF blocking is **default-on**. Disable only when crawling internal docs on a
  trusted network. Never disable in a shared environment or against untrusted
  URLs.
</Aside>
