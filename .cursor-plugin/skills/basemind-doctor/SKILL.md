...
description: >-
  Diagnose and recover hacienda-mcp when it isn't working — MCP tools missing or erroring, "no
...
# basemind-doctor — diagnose and recover hacienda-mcp

Use this when hacienda-mcp isn't behaving: MCP tools aren't available or return errors, the
statusline says **no index**, queries come back empty when they shouldn't, or the `hacienda-mcp serve`
MCP server appears dead. Every step here uses the **CLI**, so it works even when the MCP server is
down.

Important: a stdio MCP server (what `hacienda-mcp serve` is) **cannot be restarted by an agent or by
hacienda-mcp itself** — a fresh process can't resume the client's MCP `initialize` handshake.
...
- Errors / "no index" / `file_count: 0` with blobs present → the index is missing or lost. Build
  it: `hacienda-mcp scan` (see the `basemind-scan` skill / `/bm-scan`).
...
If `hacienda-mcp scan` fails with a lock error, a `hacienda-mcp serve` (or `watch`) already owns the index
for this repo. Check the lock holder:

```sh
cat .hacienda-mcp/.lock.meta 2>/dev/null   # {"command":"hacienda-mcp serve","pid":<pid>,"acquired_unix":<ts>}
```
...
- If that `pid` is **dead**, the lock is stale. The OS releases the advisory lock when a process
  dies, so a fresh `hacienda-mcp scan` / `hacienda-mcp serve` should just work — retry it. (You may delete
  the stale `.hacienda-mcp/.lock.meta` to clear the advisory holder record.)
...
hacienda-mcp can't relaunch its own stdio server; trigger a reconnect in your client:

- **Claude Code**: reconnect the hacienda-mcp MCP server from the MCP UI, or restart the session. The
  plugin's launcher re-downloads/execs the binary automatically on the next connection.
- **Cursor / others**: toggle/reconnect the hacienda-mcp MCP server in the MCP settings.

While disconnected, you are not blocked: use the `basemind-cli` skill (`hacienda-mcp query …`,
`hacienda-mcp git …`) — it reads the same `.hacienda-mcp/` index directly, no server required.

## Agent shells (embedded rmux daemon)

Only when the server was built with `--features shells`. Spawned shell sessions run under an
**embedded rmux daemon** — hacienda-mcp re-execs itself (`--__internal-daemon`) rather than shelling
...
with `HACIENDA_MCP_SHELLS_SOCKET`. It is **separate** from the comms broker daemon.
...
`hacienda-mcp serve` logs its lifecycle to stderr (captured in your client's MCP server logs):
