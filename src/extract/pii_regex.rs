...
static PATTERNS: Lazy<Vec<(&'static str, Regex)>> = Lazy::new(|| {
    ...
    vec![
        ...
        (
            "phone",
            build(r"(?:\+?\d{1,3}[\s.\-]?)?(?:\(?\d{2,4}\)?[\s.\-]?){2,4}\d{2,4}"),
        ),
        ...
        (
            "api_key",
            build(r"(?i)\b(?:sk|pk|rk)-[a-z0-9]{16,}\b|ghp_[a-zA-Z0-9]{36}|github_pat_[a-zA-Z0-9_]{22,}"),
        ),
        ("jwt", build(r"eyJ[A-Za-z0-9_\-]+\.eyJ[A-Za-z0-9_\-]+\.[A-Za-z0-9_\-]+")),
        ("pem", build(r"-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----")),
        (
            "env_assign",
            build(r#"(?i)\b(?:API_KEY|SECRET|TOKEN|PASSWORD|PASSWD|ACCESS_KEY)\s*=\s*['\"]?[^\s'"\"]{4,}"#),
        ),
    ]
});
...
mod tests {
    ...
    fn detects_sk_token_and_jwt() {
        let spans =
            detect_regex_pii("key=sk-1234567890abcdef and eyJhbGciOiJIUzI1Ni.eyJzdWIiOiIxMjM0NTY3ODkwIn0.SflKx");
        let cats: Vec<&str> = spans.iter().map(|s| s.2).collect();
