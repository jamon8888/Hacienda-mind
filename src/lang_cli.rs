//! `hacienda-mcp lang` subcommand bodies (list / install / clean). Split out of `main.rs`
...
pub(crate) fn cmd_lang_list(no_color: bool) -> Result<()> {
    ...
    let installed = hacienda_mcp::lang::downloaded_languages();
    let supported: std::collections::HashSet<&str> = hacienda_mcp::lang::SUPPORTED_LANGUAGES.iter().copied().collect();
    ...
    let _ = writeln!(out, "supported by hacienda-mcp (queries shipped):");
    for &name in hacienda_mcp::lang::SUPPORTED_LANGUAGES {
    ...
    if !extras.is_empty() {
        ...
        let _ = writeln!(
            out,
            "{d}also cached (no hacienda-mcp queries, parse-only):{r}",
            d = dim.render(),
            r = Reset.render(),
        );
        for n in extras {
    ...
    if let Some(dir) = hacienda_mcp::lang::grammar_cache_dir() {
        let _ = writeln!(out);
        let _ = writeln!(
            out,
            "{d}cache: {dir}{r}",
...
pub(crate) fn cmd_lang_install(verbosity: Verbosity, no_color: bool) -> Result<()> {
    ...
    if verbosity != Verbosity::Quiet {
        let summary = hacienda_mcp::lang::ensure_grammars().map_err(|e| anyhow::anyhow!("{e}"))?;
...
pub(crate) fn cmd_lang_clean() -> Result<()> {
    hacienda_mcp::lang::clean_grammar_cache().map_err(|e| anyhow::anyhow!("{e}"))?;
