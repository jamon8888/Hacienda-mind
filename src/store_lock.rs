//! Store write-lock machinery: the advisory `.hacienda-mcp/.lock` flock, the `.lock.meta` holder
...
/// Best-effort probe: does a live writer currently hold the exclusive `.hacienda-mcp/.lock`?
///
...
pub(crate) fn writer_lock_is_held(hacienda_mcp_dir: &Path) -> bool {
    let path = hacienda_mcp_dir.join(LOCK_FILE);
...
pub fn probe_writer_lock(hacienda_mcp_dir: &Path) -> WriterProbe {
    if writer_lock_is_held(hacienda_mcp_dir) {
        WriterProbe::Held {
            holder: read_lock_meta(hacienda_mcp_dir),
        }
    } else {
        WriterProbe::Free
    }
...
pub(crate) fn acquire_lock(hacienda_mcp_dir: &Path) -> Result<File, StoreError> {
    acquire_lock_as(hacienda_mcp_dir, LockHolder::Maintenance)
}
...
pub(crate) fn acquire_lock_as(hacienda_mcp_dir: &Path, holder: LockHolder) -> Result<File, StoreError> {
    let path = hacienda_mcp_dir.join(LOCK_FILE);
    ...
    for attempt in 0..LOCK_ATTEMPTS {
        match file.try_lock_exclusive() {
            Ok(()) => {
                write_lock_meta(hacienda_mcp_dir, holder);
                return Ok(file);
            }
...
fn write_lock_meta(hacienda_mcp_dir: &Path, holder: LockHolder) {
    ...
    let final_path = hacienda_mcp_dir.join(LOCK_META_FILE);
    let tmp_path = hacienda_mcp_dir.join(format!("{LOCK_META_FILE}.{}.tmp", std::process::id()));
...
fn read_lock_meta(hacienda_mcp_dir: &Path) -> Option<LockMeta> {
    let bytes = std::fs::read(hacienda_mcp_dir.join(LOCK_META_FILE)).ok()?;
...
mod tests {
    ...
    fn probe_writer_lock_reports_free_when_unlocked() {
        ...
        assert!(
            ...
            "an untouched .hacienda-mcp dir has no live writer"
        );
    }
    ...
    fn probe_writer_lock_names_the_live_holder() {
        ...
        match probe_writer_lock(tmp.path()) {
            WriterProbe::Held { holder: Some(meta) } => {
                assert_eq!(meta.command, "hacienda-mcp serve", "sidecar names the holder")
            }
