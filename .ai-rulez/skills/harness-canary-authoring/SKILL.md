...
- **Unambiguous by name alone** — hacienda-mcp's reference search is name-only. `get` is fine inside Django (many call sites, all the same semantic). Inside React, `get` would match too many unrelated callees; pick `useState` instead.
...
2. Confirm the count locally: clone the repo, run `hacienda-mcp scan`, call `find_references` via the MCP, note the actual count.
