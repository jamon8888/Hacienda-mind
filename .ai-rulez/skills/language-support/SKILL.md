...
Use this to add a new language to hacienda-mcp's extraction. Supported languages today include Rust, Python, TypeScript/JSX, JavaScript, Go.

## Inputs needed

- A tree-sitter grammar crate (e.g. `tree-sitter-elixir`) reachable via the `tree-sitter-language-pack` umbrella crate hacienda-mcp already depends on, or as a direct dep.
...
2. **L1 (outline) queries**
   - Adapt the upstream `tags.scm` to hacienda-mcp's L1 capture shape: `@symbol.function`, `@symbol.method`, `@symbol.class`, etc. mapped to `extract::SymbolKind`.
