...
Bumping `INDEX_SCHEMA_VER` triggers auto-wipe of `.hacienda-mcp/views/<view>/index.fjall/` on the user's next scan. The next scan rebuilds from the msgpack blobs — no data loss, but a one-time scan cost. Bump when:
...
5. **Bump `INDEX_SCHEMA_VER`** (if needed) and verify the auto-wipe path by:
   - Pointing `hacienda-mcp scan` at an existing `.hacienda-mcp/` from a prior version.
