...
Use this when adding a new `#[tool]` to hacienda-mcp's MCP server. Skipping a step leaves the tool half-wired.
...
- `poly lint .` — clean.
- `HACIENDA_MCP_HARDEN_NO_BUILD=1 cargo test --release --test harden -- --ignored --nocapture` — 8/8 green; new canary passes.
