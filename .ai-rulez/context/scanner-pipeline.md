...
`hacienda-mcp scan` is the engine. Two entry points: full scan (`scanner::scan`) and incremental (`scanner::scan_paths`). Both share the same per-file pipeline.
