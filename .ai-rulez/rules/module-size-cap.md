...
- The cap exists because hacienda-mcp already has precedent files that were split (`src/mcp/tools.rs` → `tools.rs` + `tools_<area>.rs` + `helpers.rs` + `helpers_<area>.rs`) and the split shape is the project's preferred unit of work.
