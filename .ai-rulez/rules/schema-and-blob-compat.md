...
Basemind persists state in two serialized forms: msgpack blobs under `.hacienda-mcp/blobs/<hash>.{l1,l2}.msgpack` (content-addressed) and the Fjall keyspace under `.hacienda-mcp/views/<view>/index.fjall/`. Both have explicit schema versions; both auto-wipe and rebuild on version mismatch.
...
- Wipe-on-mismatch is the explicit migration story: the next `hacienda-mcp scan` rebuilds from source. Never silently accept a mismatched version.
