...
// Resolve the hacienda-mcp binary. Prefer the cargo build output (CARGO_TARGET_DIR
// or the repo's target/), falling back to a bare `hacienda-mcp` on PATH so the
// server still starts in environments where the binary is installed globally.
function resolveServerCommand() {
  const candidates = [];
  const cargoTarget = process.env.CARGO_TARGET_DIR;
  if (cargoTarget) {
    candidates.push(path.join(cargoTarget, "debug", "hacienda-mcp.exe"));
    candidates.push(path.join(cargoTarget, "release", "hacienda-mcp.exe"));
  }
  const repoTarget = path.join(__dirname, "..", "target");
  candidates.push(path.join(repoTarget, "debug", "hacienda-mcp.exe"));
  candidates.push(path.join(repoTarget, "release", "hacienda-mcp.exe"));
  for (const c of candidates) {
    if (fs.existsSync(c)) return [c, "serve"];
  }
  return ["hacienda-mcp", "serve"];
}
